...
v2.1 是 UI 全面重设计版本，从 GitHub Primer 风升级为 **Notion 柔和风**：

- **🎨 Notion 柔和风配色**：暖灰背景层次（#f7f7f5）、暖黑文字（#37352f）、淡边框、蓝色点缀，整体气质从"工程感"变"写作感"
- **🔲 SVG 图标体系**：所有 Unicode 字符图标（☰⚙☀☾）换成统一的 24x24 线性 SVG 图标
- **📑 TabBar 圆角卡片**：去掉竖线分隔 + 底部蓝线，改用背景色区分 + 微阴影浮起
- **🔘 按钮体系统一**：primary 实心蓝 / secondary 白底框 / ghost 透明 / danger 红字，统一 6px 圆角 + 80ms hover
- **🎚️ 视图切换凹槽式**：灰底 + 白色 active 浮起（替代实心蓝底）
- **📝 排版 Notion 化**：标题无下划线、表格无竖边框/斑马纹、行内代码红字、引用块柔和底色
- **⌨️ 键盘可访问性**：TabBar/Outline 支持键盘导航，全局 focus ring
- **🎛️ 设置面板美化**：select/range/toggle 深度定制，侧栏带图标
